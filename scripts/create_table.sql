...
CREATE TRIGGER makesBookingUpdate (
	BEFORE UPDATE OF name ON Project_MaterialType pm
	FOR EACH ROW
BEGIN
	UPDATE Makes_Booking mb	
	SET mb.projectName = pm.name;
	WHERE mb.projectName = pm.name;
END;
)

CREATE TRIGGER participatesUpdate (
	BEFORE UPDATE OF name ON Project_MaterialType pm
	FOR EACH ROW
BEGIN
	UPDATE Participates p
	SET p.projectName = pm.name;
	WHERE p.projectName = pm.name;
END;
)

CREATE TRIGGER takesBookingUpdate (
	BEFORE UPDATE OF name ON Project_MaterialType pm
	FOR EACH ROW
BEGIN
	UPDATE Takes_Booking tb
	SET tb.projectName = pm.name;
	WHERE tb.projectName = pm.name;
END;
)

CREATE TRIGGER nameEducationProjectNameUpdate (
	BEFORE UPDATE OF name ON Project_MaterialType pm
	FOR EACH ROW
BEGIN
	UPDATE Name_Education_ProjectName nep
	SET nep.projectName = pm.name;
	WHERE nep.projectName = pm.name;
END;
)

CREATE TRIGGER supervisesWorksOnUpdate (
	BEFORE UPDATE OF name ON Project_MaterialType pm
	FOR EACH ROW
BEGIN
	UPDATE Supervises_WorksOn sw
	SET sw.projectName = pm.name;
	WHERE sw.projectName = pm.name;
END;
)
...
CREATE TRIGGER supervisesWorksOnUpdate (
	...
	FOR EACH ROW
BEGIN
	UPDATE Fund_ApprovedGran fa
	SET fa.projectName = pm.name;
